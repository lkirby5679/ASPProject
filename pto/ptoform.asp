...
<form method="post" action="ptoform-processor.asp" name="strForm" id="strForm">
	...
	%>			</select>
		...
		<tr>
			...
			<td><input type="Text" name="Balance" class="formstyleshort" value="<%if pto_ID<>"" then%><%=rs("Balance")%><%else%><%if Session("Balance")="" then%>0<%else%><%=Session("Balance")%><%end if%><%end if%>"></td>
		</tr>
		<tr>
			<td valign="top"><b class="boldark"><%=dictLanguage("Reason")%>:</b></td>
