...
if active = "Active" then
	sql = sql_GetActiveClientsWithRep()
else
	sql = sql_GetAllClientsWithRep()
end if
...
<table width="100%" border=0 cellpadding=2 cellspacing=2 style="border-style: solid; border-width: 2; border-color: <%=gsColorBlack%>">
	<tr>
		<td valign=top colspan=6>
			...
			<%if active="Active" then%>
				<a href="client-view.asp?active=All" class="small"><%=dictLanguage("View_All_Clients")%></a>
			<%else%>
				<a href="client-view.asp?active=Active" class="small"><%=dictLanguage("View_Active_Clients")%></a>
			<%end if%>
		</td>
	</tr>
	<tr>
