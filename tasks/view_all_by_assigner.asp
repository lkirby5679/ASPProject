...
<table border="0" cellpadding="2" cellspacing="2" width="100%">
	<tr>
		<td valign=top colspan=11>
			<b class="homeheader"><%=strShowActive%>&nbsp;<%=dictLanguage("Tasks_List_Report")%>--<%=dictLanguage("By_Assignor")%></b><br>
			<%=now()%><br>
		</td>
	</tr>
<%
sql = sql_GetTaskViewByAssignor(active)
Call RunSQL(sql, rs)
do while not rs.eof
	boolDone = rs("Done")
...
%>
	...
	<tr>
		<td colspan=11><br><b><%=dictLanguage("Created_By")%>: <%=rs("EmployeeName")%></b></td>
	</tr>
...
<%	if rs("Done") = 0 then
		...
		response.write "yes"
	end if %>&referring_page=view_all_by_assigner.asp&active=<%=active%>">
