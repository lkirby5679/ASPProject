...
<table border="0" cellpadding="2" cellspacing="2" width="100%">
	<tr>
		<td valign=top colspan=11>
			<b class="homeheader"><%=strShowActive%>&nbsp;<%=dictLanguage("Tasks_List_Report")%>--<%=dictLanguage("By_Client")%></b><br>
			<%=now()%><br>
		</td>
	</tr>

<%
sql = sql_GetTaskViewAll(active)
'Response.Write sql & "<BR>"
Call RunSQL(sql, rs)
do while not rs.eof
	boolDone = rs("Done")
	New_Client_Name = rs("Client_Name")
	if New_Client_Name <> Old_Client_Name then %>
	<tr>
		<td colspan=11><br><b><%=dictLanguage("Client")%>: <%=New_Client_Name%></b></td>
	</tr>
...
<%	if rs("Done") = 0 then
		...
		response.write "yes"
	end if%>&referring_page=view_all.asp&active=<%=active%>">
