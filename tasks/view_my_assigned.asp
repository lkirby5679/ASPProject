...
<table border="0" cellpadding="2" cellspacing="2" width="100%">
	<tr>
		<td valign=top colspan=11>
			<b class="homeheader"><%=strShowActive%>&nbsp;<%=dictLanguage("Tasks_List_Report")%>--<%=dictLanguage("Tasks_I_Assigned")%></b><br>
			<%=now()%><br>
		</td>
	</tr>
</table>
...
sql = sql_GetTaskViewByAssignorID(active, strEmpID)
Call RunSQL(sql, rs)%>
...
<table border=0 width="100%" cellpadding="2" cellspacing="2">
	<tr bgcolor="<%=gsColorHighlight%>">
		...
		<td valign=top nowrap class="columnheader"><%=dictLanguage("Assigned_To")%></td>
...
%>
	<tr <%if boolDone then%>bgcolor="#EEEEEE"<%else%>bgcolor="#FFFF77"<%end if%>>
		...
		<td valign="top" align="center" class="small">
			<a href="toggle_done.asp?id=<%=rs("Task_ID")%>&value=
<%	if rs("Done") = 0 then
		response.write "no"
	else
		response.write "yes"
	end if%>&referring_page=view_my_assigned.asp?active=<%=active%>">
...
<%	select case rs("Priority")
		...
		<td valign=top class="small"><%=rs("EmployeeNameAssignedTo")%></td>
...
<%	sqlHours = sql_GetTimecardHoursByTaskID(rs("Task_ID"))
	...
	rsHours.close
	set rsHours = nothing %>
		<td valign=top class="small"><font class="small"<%if estHoursLeft<0 then%> Color=red <%end if%>><%=estHoursLeft%></font></td>
...
<%	if boolDone = True then
		response.write "do it!"
	else
		response.write "<center><img src='" & gsSiteRoot & "images/clock.GIF' border=0></center>"
	end if %></a></td>
		<td valign=top class="small" align="center">
...
<p align="center">
<a href="new.asp"><%=dictLanguage("Add_Task")%></a><br>
