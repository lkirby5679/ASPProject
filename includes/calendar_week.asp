...
Function GetDayWeekStartsOn(dAnyDayInTheWeek)
	dTemp = DateAdd("d", -(Weekday(dAnyDayInTheWeek)-1), dAnyDayInTheWeek)
	GetDayWeekStartsOn = dTemp
End Function

Function SubtractOneWeek(dDate)
    SubtractOneWeek = DateAdd("ww", -1, dDate)
End Function

Function AddOneWeek(dDate)
    AddOneWeek = DateAdd("ww", 1, dDate)
End Function
...
iFDOW = Day(GetDayWeekStartsOn(curDate))

navDate = CDate(GetDayWeekStartsOn(curDate))
navDateLast = CDate(dateadd("d", 6, navDate)) 
	
lastweek = SubtractOneweek(curDate)
nextweek = AddOneweek(curDate) %>
	
<table BORDER="0" CELLSPACING="0" CELLPADDING="2"  width="100%" BGCOLOR="#EEEEEE">
	<tr BGCOLOR="#BBBBBB">
		<td ALIGN="left"><a HREF="calendar.asp?date=<%=lastweek%>&view=week"><img src="<%=gsSiteRoot%>images/left.gif" border="0" WIDTH="25" HEIGHT="18"></a></td>
		<td ALIGN="center" colspan="5" CLASS="homeHeader"><%=navDate%> - <%=navDateLast%></td>
		<td ALIGN="right"><a HREF="calendar.asp?date=<%=nextweek%>&view=week"><img src="<%=gsSiteRoot%>images/right.gif" border="0" WIDTH="25" HEIGHT="18"></a></td>
	</tr>
	<tr>
